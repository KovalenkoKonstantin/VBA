Attribute VB_Name = "Insertion"
Function Data_Insertion(sheet_name As String, distinct_year As Integer, decrement As Integer)
 ...
 DistinctYear = distinct_year
 Limit = 141 'последняя колонка базы
 begin = 12 'первый ряд вставки
 CompanyName = ThisWorkbook.Sheets("Preferences").Range("C7").Value2 'имя проекта
 
 Dim aw(1 To 141) As Variant
 Dim iw(1 To 141) As Variant
...
FilesToOpen = Application.GetOpenFilename _
 ...
 & CompanyName & " за " & year(Date) - decrement & " год")
...
' ----------------------------------------------------------------------------------------------------
    ...
    If Worksheets(SheetName).Cells(DataRow, i) = "% Страховых взносов ДГПХ" Then
        aw(139) = i
    End If
    If Worksheets(SheetName).Cells(DataRow, i) = "Доход в натуральной форме (ГПД)" Then
        aw(140) = i
    End If
    If Worksheets(SheetName).Cells(DataRow, i) = "Премия за объем продаж (с учетом РК)" Then
        aw(141) = i
    End If
...
' ----------------------------------------------------------------------------------------------------
    If importWB.Sheets(1).Cells(ImportFirstDataRow, i) = "% Страховых взносов ДГПХ" Then '-
        iw(139) = i
    End If
    If importWB.Sheets(1).Cells(ImportFirstDataRow, i) = "Доход в натуральной форме (ГПД)" Then '-
        iw(140) = i
    End If
    If importWB.Sheets(1).Cells(ImportFirstDataRow, i) = "Премия за объем продаж (с учетом РК)" Then '-
        iw(141) = i
    End If
...
Cells(begin, aw(K)).Select
    ActiveCell.FormulaR1C1 = _
        ...
        & """ произ. календарь'!$A:$BR"")),HLOOKUP(RC[20]," _
...
columnsToFormat = Array("EC", "ED", "EE", "EF", "EG", "DW", "DZ", "ED", "EI", "EH")

For Each col In columnsToFormat
    Columns(col & ":" & col).NumberFormat = _
        "_-* #,##0.00 _?_-;-* #,##0.00 _?_-;_-* ""-""?? _?_-;_-@_-"
Next col
