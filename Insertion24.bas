Attribute VB_Name = "Insertion24"
Sub Data_Insertion_24()
...
Cells(begin, aw(K)).Select
    ActiveCell.FormulaR1C1 = _
       "=IF(OR(CONCATENATE(RC[-1],"" 2024"")=RC[-2],VLOOKUP(RC[-2],RC1:RC108,MATCH(R8C4,R9C1:R9C108,0),0)>0,RC[4]=TRUE),"""",VLOOKUP(RC[-1],'2024 произ. календарь'!C1:C65,HLOOKUP(RC[13],'2024 произ. календарь'!R2:R3,2,0),0))"
    Cells(begin, aw(K)).Select
...
Cells(begin, aw(K)).Select
    ActiveCell.FormulaR1C1 = _
        "=IF(OR(CONCATENATE(RC[-3],"" 2024"")=RC[-4],VLOOKUP(RC[-4],RC1:RC108,MATCH(R8C4,R9C1:R9C108,0),0)>0,RC[2]=TRUE),"""",VLOOKUP(RC[-3],'2024 произ. календарь'!R18C1:R31C65,HLOOKUP(RC[11],'2024 произ. календарь'!R18:R19,2,0),0))"
    Cells(begin, aw(K)).Select
    Selection.AutoFill Destination:=Range(Cells(begin, aw(K)), Cells(iwLastRow, aw(K)))
